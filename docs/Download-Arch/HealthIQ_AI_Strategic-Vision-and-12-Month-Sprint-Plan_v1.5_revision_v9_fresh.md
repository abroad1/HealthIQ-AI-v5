...
- be able to emit non-contradictory biomarker and system-level outputs across the governed Phase 1 pathways
...
|---|---|
| Latest completed sprint (programme / sprint record) | KB-S61 |
| Canonical biomarkers in SSOT | 103 |
| Knowledge Bus package estate | 187 packages |
| Signal estate | ~234 signal definitions across package assets; ~137 merged unique runtime signals in the current SignalRegistry |
...
**Note — programme sprint row:** The “latest completed sprint” value is **governed sprint-programme bookkeeping** as of the date this revision was adopted. It is **not** a figure inferred from repository file layout or commit history. Treat it as **operational metadata** alongside the code-derived metrics in the other rows (SSOT counts, package directories, `SignalRegistry` merge behaviour).
...
Specifically: all six "Biomarker Ingestion Batch" workstream entries now reflect completed or substantially complete work. The remaining Wave 1–4 work is concentrated in the WHY expansion and structural workstreams, not the ingestion entries.
...
Status: complete. `pkg_kb45_*` packages are present in the Knowledge Bus. No further Batch 1 ingestion sprint is required unless a specific governed gap is confirmed in preflight.
...
Status: complete. `pkg_kb47_*` packages are present in the Knowledge Bus. No further Batch 2 ingestion sprint is required unless a specific governed gap is confirmed in preflight.
...
Status: complete. The Batch 3–4 domain surface was promoted via KB-S52C and KB-S52D (Wave A/B remap). `pkg_kb52c_*` and `pkg_kb52d_*` packages are present. No further Batch 3 ingestion sprint is required unless a specific governed gap is confirmed in preflight.
...
Status: complete. Absorbed into the Wave A/B remap ingestion (see KB-S49 note above).
...
Status: substantially complete. Pass 3 ingestion was executed across CBC (KB-S58), thyroid antibodies (KB-S59), lipid derived (KB-S60), and transferrin (KB-S61). Remaining breadth work, if any, concerns specific Wave C domain gaps identified in the KB-S53 classification audit and the KB-S57 CBC tranche blocker report, not a sequential "Batch 5." A cold sprint author must read `KB-S53_WAVE_C_CLASSIFICATION_AUDIT.md` and `KB-S57_CBC_TRANCHE_BLOCKER_REPORT.md` in preflight to identify any remaining ungated artefacts before authoring any further ingestion sprint.
...
Status: KB-S57 produced a blocker report (Outcome B — no v3 governed specs available at that time). Subsequent Pass 3 ingestion (KB-S58–KB-S61) addressed the resolved domains. Any remaining CBC tranche work must be assessed against the KB-S57 blocker report and current research artefact state in preflight before an ingestion sprint is authored.
