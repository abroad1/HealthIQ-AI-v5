...
# HealthIQ AI — Final Recommendation Paper v3
...
| Patterns across your body | structured interpretation | current system/cluster naming; phenotype display layer where confirmed | system explainers, supporting marker summaries | **Phase 2 — existence check gate required before speccing** |
...
6. Pattern naming must be strong and three-layered where contract support is confirmed — not assumed universally.
...
9. Rich KB explanation fields must be treated as selective, not universal — always support a fallback path.
10. Gemini is optional polish, not the product spine.
11. Clinician handoff remains separate from the retail journey.
12. ExplainabilityReportV1 is an internal audit artifact — do not treat it as a consumer content source without a dedicated compiler step.
13. Section 5 (patterns layer) is Phase 2 — it must not be specced or sprinted without completing the existence check gate first.
...
**Success criterion:**
- At least the most universally computed Layer C feature (to be determined by backend team) is live for all analysis results
...
- Phase 1 (Sprints R-1 through R-5) is complete and stable in production
...
| R-2 | Section 3 — primary finding | Frontend + minor backend check | None | Verify top_hypothesis_line, chains[] in DTO |
| R-3 | Section 4 — uncertainty | Frontend + minor backend check | None | Verify runner_up fields in DTO |
...
| R-6 | Section 7 — Layer C insights | Frontend + backend feature extraction | None | Extract layer_c_features from meta into DTO |
| R-7 | Section 5 gate | Research + specification only | None | None (decision sprint) |
| R-8 | Section 5 — patterns layer | Frontend (or backend contract first) | None | Conditional on R-7 outcome |
