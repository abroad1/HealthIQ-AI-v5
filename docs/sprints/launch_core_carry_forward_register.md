...
As of KB-UTIL-2-PROMOTE-PILOT closure (2026-06-02), there are no known launch-blocking carry-forwards. One ROUTE_A package candidate was validated with advisory fixes closed (compile manifest source_contract_version + governed schema_version 1.0.0 decision), but runtime activation is deferred (`compiled_not_promoted`) due duplicate activation-key collision with existing canonical authority. CF-KBUTIL1-001 remains open until promotion harness/runtime authority wiring and de-dup authority switching are completed.
