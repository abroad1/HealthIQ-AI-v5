...
| CF-MRIMPROVE-001 | CRP-PASS3-MIGRATION | Re-review non-Pass_3 runtime packages through Knowledge Bus | Some runtime-active packages were not generated through the current Pass_3 process. They should be internally flagged for Knowledge Bus re-review so we can confirm, update, replace or retire them before treating them as mature launch intelligence. No user-facing disclosure is required. | No | MED-RESEARCH-REVIEW-1_non_pass3_package_revalidation | Resolved | Resolved by MED-RESEARCH-REVIEW-1 — all 55 packages classified in `non_pass3_package_revalidation_register_v1.yaml`; zero launch_visible; no launch blocker. |
| CF-MRIMPROVE-002 | CRP-PASS3-MIGRATION | `pkg_kb45_*` pre–Pass 3 batch JSON lineage | Ten `pkg_kb45_*` packages cite `investigation-spec-collection-batch*.json` (not Pass 3 JSON). Ambiguous provenance vs Pass 3 estate. | No | KB-UTIL-2 / Pass 3 mapping | Deferred | MED-RESEARCH-REVIEW-1 — `batch_json_lineage_review_required`; action `map_to_existing_pass3_frame`. |
| CF-MRIMPROVE-003 | CRP-PASS3-MIGRATION | Architecture-doc anchor package cohort | Eight context packages cite `docs/architecture/HealthIQ_Investigation_Layer.md` only. Runtime-loaded thin context signals. | No | KB-UTIL-2 / investigation extraction | Deferred | MED-RESEARCH-REVIEW-1 — `internal_only`; action `map_to_existing_pass3_frame`. |
| CF-MRIMPROVE-004 | CRP-PASS3-MIGRATION | `pkg_lipid_transport` provenance gap | Package manifest lacks `source_document`; provenance_gap classification. | No | KB hygiene | Open | MED-RESEARCH-REVIEW-1 — `recover_provenance`; overlaps KBP-0001 thresholds; not launch_visible. |
...
As of MED-RESEARCH-REVIEW-1 closure (2026-05-31), there are no known launch-blocking carry-forwards. CF-MRIMPROVE-001 is resolved (55-package audit). CF-MRIMPROVE-002–003 are deferred with classified cohorts. CF-MRIMPROVE-004 (`pkg_lipid_transport` provenance) remains open. Pass 3 CRP compile (CF-CRPPASS3-001), chronic inflammation frame (CF-CHRONICINFL-001), and root-cause programme (CF-ARCHLEG1-001 / CF-ARCHLEG1-004) remain open.
