...
| CF-BATCH2-010 | BATCH2-REMAINDER-RESOLUTION-1 | Androgen-panel clinical sign-off before activation | All 8 androgen packages EXCLUDED from promotion; no clinical sign-off artefact in repo. | No | Androgen clinical sign-off sprint | Open | All 8 remain inactive. Clinical sign-off required before activation. Pipeline context threading complete (CONTEXT-THREADING-1); semantics correction still required (CF-CONTEXT-SEMANTICS-1). Register: `batch2_remaining_blockers_execution_register_v1.yaml`. |
| CF-CONTEXT-MOD-3 | BATCH2-REMAINDER-RESOLUTION-1 | Implement runtime Layer B context modifier evaluation after androgen governance binding | Investigation confirms all catalogue modifiers runtime_active=false; blocks all 8 androgen packages and FT3 low illness/medication context. | No | — | Resolved | Resolved by CONTEXT-RUNTIME-1 + CONTEXT-THREADING-1 — reusable evaluator + package gates live in pipeline via raw `questionnaire_data` bridge at Step 1.6. FT3 low + 8 androgen packages remain inactive; activation blocked separately (CF-BATCH2-010, CF-CONTEXT-SEMANTICS-1). |
