...
| CF-SENTINEL-001 | ARCH-SENTINEL-1 | Wire medical-intelligence sentinel into standard CI / Automation Bus gate | Validator and pytest sentinels exist; not yet a required CI job on every PR. | No | CI-ARCH-GATE-1 | Open | Integrate `validate_medical_intelligence_architecture.py` and `test_medical_intelligence_architecture_sentinels.py` into default bus validation profile. |
...
As of ARCH-SENTINEL-1 closure (2026-06-02), there are no known launch-blocking carry-forwards. Medical frame identity index covers creatinine, ALT, CRP, and ferritin_high (18 frames total). ARCH-SENTINEL-1 adds medical-intelligence architecture sentinels (`validate_medical_intelligence_architecture.py`, pytest module, day-one validator delegation). Bulk ROUTE_A promotion remains paused (CF-PASS3FRAME-003). Pass_3 enrichment (CF-PASS3FRAME-002, CF-CREATININE-001, CF-CRPPASS3-001) and Layer B modifiers (CF-CONTEXT-MOD-2) remain open. CI wiring for sentinels tracked as CF-SENTINEL-001. No runtime behaviour changes.
