...
| CF-BATCH2-007 | BATCH2-REMAINDER-RESOLUTION-1 | Adjudicate Wave D egfr multi-frame candidates | Independent signal_egfr_low activation risks duplicate renal signalling vs creatinine canonical + legacy eGFR override. | No | BATCH2-EGFR-AUTHORITY-1 | Resolved | Resolved by BATCH2-EGFR-AUTHORITY-1 — reusable `signal_authority_collision_model_v1.yaml` adjudicates renal_filtration_axis; eGFR packages authority-classified and runtime-activated via CF-AUTHORITY-RUNTIME-1 + CF-AUTHORITY-RUNTIME-1B. Register: `batch2_egfr_authority_execution_register_v1.yaml`, `authority_runtime_execution_register_v1.yaml`. |
...
| CF-BATCH2-013 | BATCH2-THYROID-GATE-1 | Thyroid TSH-gated activation logic required before thyroid runtime activation | Mandatory pre-emission TSH gates implemented in SignalEvaluator + 3 package signal libraries; 11/11 regression tests PASS; Phase 3 metadata activation after APPROVE BATCH2 THYROID GATED ACTIVATION. | No | — | Resolved | Resolved by BATCH2-THYROID-GATE-1 Phase 3 — 3/3 eligible thyroid packages runtime_active_canonical with mandatory TSH pre-emission gates; FT3 low deferred (thyroid_activation_requires_tsh_ft4_and_illness_medication_context). Register: `batch2_thyroid_gate_execution_register_v1.yaml`. |
| CF-BATCH2-010 | BATCH2-REMAINDER-RESOLUTION-1 | Androgen-panel clinical sign-off before activation | All 8 androgen packages EXCLUDED from promotion; no clinical sign-off artefact in repo. | No | Androgen clinical sign-off sprint | Open | All 8 androgen packages clearance decision BLOCKED_PENDING_CLINICAL_SIGNOFF per `batch2_context_clearance_register_v1.yaml`. BATCH2-MINIMUM-COVERAGE-1 formalised minimum-coverage decision DEFER_PENDING_EXTERNAL_CLINICAL_AUTHORITY for all 8; zero activations. External sign-off workflow required before BATCH2-ANDROGEN-EXECUTION-1. Register: `batch2_minimum_coverage_decision_register_v1.yaml`. |
| CF-CONTEXT-MOD-3 | BATCH2-REMAINDER-RESOLUTION-1 | Implement runtime Layer B context modifier evaluation after androgen governance binding | Investigation confirms all catalogue modifiers runtime_active=false; blocks all 8 androgen packages and FT3 low illness/medication context. | No | — | Resolved | Capability + threading complete (CONTEXT-RUNTIME-1, CONTEXT-THREADING-1). Package activation governed separately via CONTEXT-CLEARANCE-1 clearance register; 0 packages activation-eligible. |
| ARCH-ORCH-RESTRUCTURE-1 | CONTEXT-THREADING-1 | Restructure orchestrator phase ordering for final day-one architecture | Signal evaluation still runs before full AnalysisContext creation; CONTEXT-THREADING-1 uses raw questionnaire_data as interim bridge only. | No | Day-one architecture / launch gate | Resolved | Resolved by ARCH-COMPLETION-1 — AnalysisContext is assembled before signal evaluation; runtime context is derived via `build_runtime_context_snapshot_from_analysis_context()` from governed post-context fields rather than raw `questionnaire_data` bridge. |
| CF-CONTEXT-SEMANTICS-1 | CONTEXT-THREADING-1 | Resolve androgen runtime_context_requirements semantics before activation | Current gates treat hormone_therapy and aas_exposure as hard presence requirements; medical review may require disclosed-context vs hard-gate distinction. | No | — | Resolved | Resolved by CONTEXT-CLEARANCE-1 (taxonomy) + BATCH2-CONTEXT-COMPLETION-1 (runtime disclosed mode + package metadata remediation). Evaluator supports `disclosed` requirement mode; misclassified package metadata aligned. Activation remains STOP-gated; 0 packages activated. |
| CF-AUTHORITY-RUNTIME-1 | BATCH2-EGFR-AUTHORITY-1 | Implement runtime consumption of signal_authority_collision_model_v1.yaml | Reusable authority/collision model adjudicated governance-only; SignalEvaluator had no anti-double-counting mechanism. | No | — | Resolved | Resolved by CF-AUTHORITY-RUNTIME-1 — renal_filtration_axis enforced via signal_authority_collision_resolver.py post-processing in evaluate_all(); regression tests PASS. eGFR Phase 3 runtime activation completed by CF-AUTHORITY-RUNTIME-1B after APPROVE BATCH2 EGFR AUTHORITY ACTIVATION. Register: `authority_runtime_execution_register_v1.yaml`. |
