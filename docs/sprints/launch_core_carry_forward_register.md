...
| CF-BATCH2-007 | BATCH2-PROMOTION-READINESS-1 | Adjudicate Wave D egfr multi-frame candidates | 2 egfr frames require adjudication vs creatinine legacy eGFR override before promotion. | No | egfr frame adjudication sprint | Open | Both pkg_kb47_egfr_* packages excluded from BATCH2-PROMOTE-1 scope (CF-BATCH2-CLOSURE-1). |
...
| CF-BATCH2-010 | BATCH2-MEDREVIEW-1 | Androgen-panel clinical sign-off before activation | Medical review complete but no frame cleared for activation without clinical adjudication. | No | Clinical sign-off / promotion gate sprint | Open | All 8 androgen packages excluded from BATCH2-PROMOTE-1 (CF-BATCH2-CLOSURE-1). |
...
| CF-BATCH2-012 | BATCH2-ACTIVATION-1 | Runtime activation of cleared non-thyroid Batch 2 promoted subset | 6 creatine_kinase/eosinophil packages preflight-cleared; STOP gate pending human approval. | No | BATCH2 runtime activation (post-approval) | Open | Deferred pending approval phrase `APPROVE BATCH2 RUNTIME ACTIVATION`. BATCH2-ACTIVATION-1 preflight PASS; validators 6/6 PASS; no runtime authority changes performed. |
| CF-BATCH2-013 | BATCH2-ACTIVATION-1 | Thyroid TSH-gated activation logic required before thyroid runtime activation | 4 thyroid packages governance-promoted; clinical sign-off APPROVE_SUBSET ingested (`docs/Medical Research Documents/thyroid_blood_marker_interpretation_clinical_signoff.md`). TSH mandatory for all four; FT3 low additionally requires TSH + FT4 + illness/medication context. | No | Thyroid TSH-gated activation logic sprint | Open | Sign-off received; runtime activation blocked until TSH-gating logic implemented (CF-BATCH2-013). |
| CF-CONTEXT-MOD-3 | BATCH2-CONTEXT-MOD-1 | Implement runtime Layer B context modifier evaluation after androgen governance binding | Androgen-panel catalogue links exist but runtime_active=false; identity index flags set without runtime consumption. | No | CONTEXT-MOD-2 / runtime binder sprint | Open | Implement modifier evaluation binder per CF-CONTEXT-MOD-2 scope; androgen panel is first bound cohort. |
