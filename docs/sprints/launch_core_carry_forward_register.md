...
| CF-MEDFRAME1-001 | MED-FRAME-1 | Medical frame identity index and family registry | Architecture defined in `docs/architecture/MED-FRAME-1_signal_family_contextual_frame_architecture.md`; needs machine-enforced index for promotion and collision checks. | No | MED-FRAME-2 | Open | Draft sketch: `knowledge_bus/governance/medical_frame_identity_model_draft_v1.yaml` (non-runtime). |
...
As of MED-FRAME-1 closure (2026-06-02), there are no known launch-blocking carry-forwards. Signal-family / medical-frame architecture is documented to prevent flat-signal collapse and to bind questionnaire and medication context as structured medical inputs. Implementation carry-forwards opened: MED-FRAME-2 (frame index), CONTEXT-MOD-1 (modifier governance), and creatinine multi-frame adjudication (CF-MEDFRAME1-003). Package-promotion sprints are not marked resolved by this architecture-only sprint.
