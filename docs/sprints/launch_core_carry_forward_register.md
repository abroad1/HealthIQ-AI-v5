...
| CF-BATCH2-012 | BATCH2-PROMOTE-1-CONTINUATION | Runtime activation of cleared non-thyroid Batch 2 promoted subset | 6 creatine_kinase/eosinophil packages preflight-cleared; STOP gate pending human approval. | No | BATCH2 runtime activation (post-approval) | Open | Deferred pending approval phrase `APPROVE BATCH2 RUNTIME ACTIVATION`. Register: `batch2_runtime_activation_execution_register_v1.yaml`. Phase 1 preflight PASS; no activation performed. |
| CF-BATCH2-013 | BATCH2-PROMOTE-1-CONTINUATION | Thyroid clinical sign-off required before thyroid runtime activation | 4 thyroid packages governance-promoted but runtime-deferred; no sign-off record in repo. | No | Thyroid clinical sign-off sprint | Open | Deferred within BATCH2-PROMOTE-1-CONTINUATION; activation blocked until clinical sign-off documented. |
