...
As of KB-UTIL-2-PROMOTE-PILOT closure (2026-06-02), there are no known launch-blocking carry-forwards. One ROUTE_A package (`pkg_creatinine_high_renal_pass3_v1`) was promoted from generated pilot artefacts with validation pass and advisory fixes closed (compile manifest source_contract_version + governed schema_version 1.0.0 decision). Runtime activation remains deferred (`compiled_not_promoted`), and CF-KBUTIL1-001 remains open until promotion harness/runtime authority wiring is completed for broader rollout.
