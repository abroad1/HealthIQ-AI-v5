...
| CF-SENTINEL-001 | ARCH-SENTINEL-1 | Wire medical-intelligence sentinel into standard CI / Automation Bus gate | Validator and pytest sentinels exist; not yet a required CI job on every PR. | No | CI-ARCH-GATE-1 | Resolved | Resolved by CI-ARCH-GATE-1 — `run_architecture_validation_gate.py`, `golden_gate_local.py` first check, `.github/workflows/architecture-gate.yml` on main/develop. |
...
As of CI-ARCH-GATE-1 closure (2026-06-02), there are no known launch-blocking carry-forwards. Medical-intelligence sentinels run via `run_architecture_validation_gate.py` in Automation Bus golden gate and GitHub `architecture-gate.yml` (CF-SENTINEL-001 resolved). Bulk ROUTE_A promotion remains paused (CF-PASS3FRAME-003). Pass_3 enrichment and Layer B modifiers remain open. No runtime behaviour changes.
