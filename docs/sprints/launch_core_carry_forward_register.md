...
| CF-ARCHLEG1-004 | ARCH-LEGACY-1 | Extend ARCH-RT-6 validator for legacy retirement gaps | `validate_arch_legacy2_retirement` and stricter `validate_wave1_assembler_routing` added. CRP/root-cause inventory guards remain deferred. | No | CRP-PASS3-MIGRATION / ARCH-RT-4+ | Open | Partial — ARCH-LEGACY-2 implemented bounded retirement guards only. |
...
As of ARCH-LEGACY-2 closure, there are no known launch-blocking carry-forwards. Open items include root-cause/CRP migration (CF-ARCHLEG1-001/002), partial validator hardening for CRP/root-cause inventory (CF-ARCHLEG1-004), compile automation, hypothesis surfacing, NarrativePayload persistence, lineage, and test-hardening improvements.
