...
| CF-BATCH2-010 | BATCH2-REMAINDER-RESOLUTION-1 | Androgen-panel clinical sign-off before activation | All 8 androgen packages EXCLUDED from promotion; no clinical sign-off artefact in repo. | No | Androgen clinical sign-off sprint | Open | Formal runtime blocker `androgen_runtime_context_evaluation_required` recorded by BATCH2-REMAINING-BLOCKERS-1. All 8 remain inactive. Clinical sign-off still required before re-promotion/activation after CF-CONTEXT-MOD-3. Register: `batch2_remaining_blockers_execution_register_v1.yaml`. |
| CF-CONTEXT-MOD-3 | BATCH2-REMAINDER-RESOLUTION-1 | Implement runtime Layer B context modifier evaluation after androgen governance binding | Investigation confirms all catalogue modifiers runtime_active=false; blocks all 8 androgen packages and FT3 low illness/medication context. | No | — | Resolved | Resolved by CONTEXT-RUNTIME-1 — reusable `runtime_context_evaluator.py` + package-declared `runtime_context_requirements` enforced fail-closed in SignalEvaluator.evaluate_all(); 13/13 regression tests PASS. FT3 low + 8 androgen packages remain inactive pending orchestrator context threading and human STOP. Register: `context_runtime_execution_register_v1.yaml`. Follow-on: thread runtime_context from orchestrator questionnaire merge. |
