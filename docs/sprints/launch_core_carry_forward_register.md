...
As of KB-UTIL-2-PROMOTE-WIRE-1 closure (2026-06-02), there are no known launch-blocking carry-forwards. Creatinine runtime authority preflight is now explicit: runtime loading uses activation-key identity from `knowledge_bus/packages`, and canonical Pass_3 frame authority for `signal_creatinine_high::inv_creatinine_high_reduced_glomerular_filtration` remains `pkg_kb52c_creatinine_high_reduced_glomerular_filtration`. The promoted candidate remains `compiled_not_promoted` (non-activation decision) to avoid duplicate Pass_3 frame authority. Legacy s24 divergence adjudication remains open before any full authority convergence.
