...
| CF-AUTHORITY-RUNTIME-1 | BATCH2-EGFR-AUTHORITY-1 | Implement runtime consumption of signal_authority_collision_model_v1.yaml | Reusable authority/collision model adjudicated governance-only; SignalEvaluator has no anti-double-counting mechanism. | No | AUTHORITY-RUNTIME-1 or Intelligence Core sprint | Open | Prerequisite before eGFR Batch 2 metadata activation. Wire declarative authority_resolution pattern from `signal_authority_collision_model_v1.yaml` renal_filtration_axis with fail-closed duplicate suppression. Register: `batch2_egfr_authority_execution_register_v1.yaml`. |
