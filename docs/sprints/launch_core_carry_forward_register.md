...
| CF-BATCH2-010 | BATCH2-REMAINDER-RESOLUTION-1 | Androgen-panel clinical sign-off before activation | All 8 androgen packages EXCLUDED from promotion; no clinical sign-off artefact in repo. | No | Androgen clinical sign-off sprint | Open | All 8 androgen packages clearance decision BLOCKED_PENDING_CLINICAL_SIGNOFF per `batch2_context_clearance_register_v1.yaml`. Semantics defined (CF-CONTEXT-SEMANTICS-1 resolved); activation remains blocked until clinical sign-off artefact exists. |
...
| ARCH-ORCH-RESTRUCTURE-1 | CONTEXT-THREADING-1 | Restructure orchestrator phase ordering for final day-one architecture | Signal evaluation still runs before full AnalysisContext creation; CONTEXT-THREADING-1 uses raw questionnaire_data as interim bridge only. | No | Day-one architecture / launch gate | Open | Before final day-one architecture acceptance, review/restructure orchestrator so context assembly precedes context-dependent evaluation. Thin runtime loaders should receive governed, assembled inputs — not reach backwards into raw questionnaire payloads. |
| CF-CONTEXT-SEMANTICS-1 | CONTEXT-THREADING-1 | Resolve androgen runtime_context_requirements semantics before activation | Current gates treat hormone_therapy and aas_exposure as hard presence requirements; medical review may require disclosed-context vs hard-gate distinction. | No | — | Resolved | Resolved by CONTEXT-CLEARANCE-1 — `runtime_context_semantics_model_v1.yaml` distinguishes hard_prerequisite_gate, disclosed_context_requirement, interpretation_modifier, companion_biomarker_requirement; `batch2_context_clearance_register_v1.yaml` documents per-package clearance. Evaluator/package metadata remediation deferred to BATCH2-CONTEXT-ACTIVATION-1. |
