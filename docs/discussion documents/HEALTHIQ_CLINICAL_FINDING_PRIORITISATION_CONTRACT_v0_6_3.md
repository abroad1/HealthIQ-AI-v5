...
# HealthIQ Clinical Finding Prioritisation Contract v0.6.3

> **v0.6.3 revision note — final targeted correction.** Three corrections only. No clinical or product position is reopened.
>
> 1. **Vitamin D contextual handling corrected.** The v0.6.2 package allowed vitamin D to be nested as an aetiological contributor to hypocalcaemia *at any concentration, including a normal result*. That was clinically wrong and is removed. Nesting as a plausible contributing factor is now permitted only below 25 nmol/L; 25–50 nmol/L may appear as limited context and may not be described as proven deficiency or as an established cause; above 50 nmol/L it may only be used to state that vitamin D deficiency is not supported by the available result.
> 2. **Vitamin D evidence attribution corrected.** The governed UK evidence supports the `<25 nmol/L` threshold. The dispositions at 25–50 nmol/L and above 50 nmol/L are **HealthIQ policy dispositions**, not source-defined diagnostic categories, and are now labelled as such.
> 3. **Status updated** to `CLINICALLY_RATIFIED_FOR_ARCHITECTURE_HARDENING`.
>
> The new status means the clinical package is ready for architecture hardening. It does **not** authorise implementation or release; `implementation_status: NOT_AUTHORISED` is retained.
>
...
Attribution: the governed UK evidence supports `<25 nmol/L` as the threshold below which risk of poor musculoskeletal health increases. HealthIQ does not create an independent finding at `25–50 nmol/L` in this version and creates no vitamin D concern above `50 nmol/L`. Those two dispositions are HealthIQ policy, not source-defined diagnostic categories.
...
`CLINICALLY_RATIFIED_FOR_ARCHITECTURE_HARDENING`

The clinical package is ratified and ready for architecture hardening. This status does **not** authorise implementation or release. `implementation_status: NOT_AUTHORISED` is retained, and the governance conditions in §23.6 continue to apply.
...
v0.6.3 additionally records:

- vitamin D may be nested as a plausible contributing factor to hypocalcaemia **only below 25 nmol/L**; a normal or insufficient result may not be presented as an aetiological contributor;
- the governed UK evidence supports the `<25 nmol/L` threshold; the 25–50 nmol/L and >50 nmol/L dispositions are **HealthIQ policy**, not source-defined diagnostic categories;
- package status is `CLINICALLY_RATIFIED_FOR_ARCHITECTURE_HARDENING`.
...
1. Architecture hardening of the ratified clinical package — contract v0.6.3, consolidated ruleset v0.5, adjudication register v0.4, closure report v0.4.
...
3. Continue specialist regulatory review in parallel. R1, R5 and R6 remain blocking for release.
4. Retain the hepatic ruleset as parked domain evidence pending haematology dependencies.
5. Progress the questionnaire rationalisation carry-forward, which remains release-blocking.
6. **Do not author implementation work until the governance conditions in §23.6 are met.** Architecture hardening does not authorise implementation.
