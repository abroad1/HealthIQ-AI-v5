...
version: "0.4"
status: READY_FOR_CONFIRMATORY_INDEPENDENT_MEDICAL_REVIEW
...
# HealthIQ Clinical Finding Prioritisation Contract v0.4
...
No universal arithmetic rule for converting indeterminate severity into a concern tier is authorised in v0.4.
...
Where a governed domain rule identifies a marker as uninterpretable without a named modifier, absence of that modifier produces an insufficient-data output for that finding rather than a low-confidence interpretation. Such marker–modifier pairs must be explicitly enumerated by domain.
...
The output must lead with the assessment limitation, not with a reassuring findings summary.
...
`READY_FOR_CONFIRMATORY_INDEPENDENT_MEDICAL_REVIEW`

Basis:

- the core model was validated across hepatic, electrolyte, renal, haematology, iron, thyroid/endocrine, cardiometabolic, inflammatory and nutritional domains;
- A1–A6 from the cross-domain validation have been incorporated;
- no universal arithmetic tier formula for indeterminate severity has been authorised;
- the hepatic “any abnormality equals Tier 1” principle is explicitly prohibited from universalisation;
- haematology is now the first detailed domain.

Next actions:

1. Short confirmatory independent medical review of v0.4 against the cross-domain validation report.
2. Head of Medical Research reconciliation of any sentence-level defects.
3. Anthony confirmation only if the review identifies a new product decision.
4. Author the Haematology Clinical Prioritisation Pilot Specification.
5. Continue specialist regulatory review in parallel.
6. Do not author implementation work until the governance conditions in §23.6 are met.
