# HealthIQ AI — Pre-SOP Prompt Scoping Workflow v0.5
...
**Supersedes:** v0.4  
**Changes from v0.4:** §6 only — mandatory derivation sequence before sprint list; product-outcome-first output format; anti-micro-sprint gate with reference definitions including precondition-bundling rule.
...
### Mandatory derivation sequence before writing the sprint list

Before producing the sprint sequence, Claude must complete these three steps in order. The output of each step must appear in the advisory.

**Step 1 — Identify product outcomes.**
State what domains, capabilities, or production signals become available at the end of the proposed sequence. Use capability language ("thyroid domain card active", "49 → 53 production opt-ins"), not carry-forward language ("cf_007 resolved"). Carry-forwards are inputs to the reasoning, not the unit of sequencing.

**Step 2 — Derive minimum sprint count.**
Working backwards from the product outcomes in Step 1, state the minimum number of sprints required to deliver them. Commit to that number. Any sequence containing more sprints than this minimum requires one sentence of written justification per surplus sprint explaining why bundling is not safe.

**Step 3 — Apply the anti-micro-sprint gate to each proposed sprint.**
For every sprint in the proposed sequence, answer: *"Does this sprint deliver standalone runtime or product output?"* A sprint that produces only a governance document, ADR, report, or configuration entry without accompanying runtime implementation fails the gate and must be absorbed into the adjacent implementation sprint unless execution-safety requires separation. If absorbed, record the absorption decision and reason.

These three steps are mandatory. An advisory that skips them and proceeds directly to a sprint list is incomplete.
...
Sprint 1: [theme] | Agent: [x] | Scope: [one line] | Dependency: [none or prior sprint] | Product output: [one line — what runtime or user-facing output this sprint delivers]
...
### Anti-micro-sprint gate — reference definitions

A sprint **passes** the anti-micro-sprint gate if it delivers at least one of:

- A new domain active at runtime (domain card, assembler, subsystem routing)
...
- A sprint that creates a necessary runtime precondition AND bundles the immediately unlocked capability into the same sprint where safe to do so
- A blocker cleared AND the unlocked capability implemented in the same sprint

A sprint **fails** the gate (micro-sprint) if it produces only:
...
A failed sprint must be absorbed into the adjacent implementation sprint. Only two exceptions permit a standalone governance sprint:
1. The governance decision is architecturally load-bearing and cannot safely be bundled (must be stated explicitly).
2. The implementation sprint would become unsafe in scope if the governance decision were included (must be stated explicitly with scope estimate).
