...
- assign named QA/UAT owner — **still open**
- sign off at §5 — pending QA/UAT owner assignment
...
| QA/UAT and proving decisions | **Named human — to be assigned by Anthony** | **OPEN — Anthony to name** |
| Programme/docs coordination | Anthony Broad (or named delegate) | Assigned unless delegated |

**One action required from Anthony:** name the QA/UAT owner before gate closes.
...
| Field | Entry |
|---|---|
| Owner | QA/UAT owner (to be named by Anthony) — owned until named |
...
| Ownership model assigned | **OPEN — one item** | Role-based owners confirmed. Named human for QA/UAT still required from Anthony. This is the only remaining open item. |
...
| Gate signed off | **OPEN — one item pending** | QA/UAT named human must be assigned by Anthony before formal gate closure and §5 sign-off. |
...
| Sign-off | **Pending — all ten decisions recorded. One item remains open: QA/UAT named human (§2). Gate formally closes and Sprint 1 can be authored once that assignment is made.** |
| Date closed | *(pending — one item outstanding)* |
| Gate outcome | *(pending — one item outstanding)* |
| Notes for Sprint 1 authoring | All decisions needed to author Sprint 1 are recorded. Sprint 1 can proceed on the following confirmed scope: (1) WHY assets for the §3.1 biology slice — targeted completion for the proving slice only. (2) No statin modifier prerequisites — deferred to pre-Sprint 2 gate. (3) Questionnaire reduced to minimum proving set. (4) `insights[]` retirement initiated (implementation Sprint 4, but decision recorded now). (5) Mock-mode honesty wording confirmed for Sprint 4. (6) All 6 acceptance criteria confirmed; Check 3 gated on pre-Sprint 2 statin gate. |

---

## 6. Immediate next step after gate closure

Once this gate is closed (QA/UAT owner named), the next artefact is the **Sprint 1 work package** for launch-core analytical hardening.
