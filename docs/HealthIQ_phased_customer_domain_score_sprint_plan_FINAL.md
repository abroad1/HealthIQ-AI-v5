# HealthIQ AI — Phased Customer Domain Score Sprint Plan v1.1
...
### 4. Confidence must ship with the score
No customer-facing domain score should be exposed without a confidence level and confidence explanation.
...
- active signals
- IDL content
- insights / narrative_report_v1 / root-cause missing-data structures
...
### 7. Do not mix scoring tracks without an explicit rule
The plan must explicitly distinguish between:
- scoring-policy range-position rails
- burden / capacity rails

They use different calibrations and cannot be blended casually.
...
## Phase 1 Sprint 1 — Wave 1 domain score + confidence contract and assembler

### Objective
Create the deterministic backend translation layer for the first three domains, including confidence from day one.
...
- explicit confidence logic for the first three domains
- no frontend exposure until score + confidence are both complete
...
- domain score object containing at minimum:
  ...
  - any raw source ids needed for later narrative assembly
...
- Cardiovascular health:
  ...
  - do not casually blend inflammatory burden or homocysteine into the numeric score without an explicit rule
- Blood sugar control:
  - use current metabolic scoring rail
  - explicitly keep blood sugar domain separate from broader metabolic burden score
  - do not imply a governed “prediabetes override” unless implementation is explicitly authored and approved
- Liver health:
  ...
  - scoring key is `liver`; burden/capacity key is `hepatic` — this must be handled explicitly
...
unless the blend rule is domain-specific, explicit, and documented.

### Acceptance criteria
- all three domains have score + confidence together
- no frontend exposure yet
- any ambiguous mappings are explicitly documented
- `liver` vs `hepatic` mapping is handled correctly
- confidence tier vocabulary is explicitly defined, including any mapping from engine 3-tier confidence to 4-tier product confidence if that route is chosen
...
- confidence model is bundled in from the start

---

## Phase 1 Sprint 2 — Wave 1 narrative contract implementation
...
### Important boundary
Sprint 1 creates the contract shape and raw evidence surfaces.
Sprint 2 assembles the user-facing narrative sentences from those raw deterministic sources.

Sprint 2 is not score generation work.
...
- IDL `why_it_matters`
- `insights`
- `narrative_report_v1` where appropriate
- root cause missing-data structures
- governed next-step sources

### Key known gap to address
The cardiovascular lipid-dominant consequence gap must be fixed here.

### Deliverables
- fully assembled domain narrative payloads for the first 3 domains
- governed cardiovascular lipid-dominant consequence line available for the Wave 1 path

### Gap assignment
This sprint explicitly owns the Wave 1 cardiovascular content gap:
- add governed lipid-dominant cardiovascular `why_it_matters`
- likely via low-risk content update to the relevant IDL asset or another approved governed source
...
## Phase 1 Sprint 3 — Frontend domain card UX (Wave 1)
...
## Phase 1 Sprint 4 — Wave 1 UAT and calibration review
...
These three are logically grouped in the roadmap, but implementation work on them is largely independent and can run in parallel if delivery speed matters.

---

## Phase 2 Sprint 5 — Blood, iron & oxygen aggregation design and implementation
...
## Phase 2 Sprint 6 — Kidney function enhancement
...
## Phase 2 Sprint 7 — Thyroid scoring rail and domain implementation
...
## Phase 2 Sprint 8 — Frontend extension from 3 to 6 domains
...
## Phase 2 Sprint 9 — Full 6-domain UAT and comparative product review
...
## Phase 3 Sprint 10 — Silent inflammation feasibility review
...
## Phase 3 Sprint 11 — Hormone balance feasibility review
...
### Build first
1. Wave 1 domain score + confidence contract and assembler
2. Wave 1 narrative contract implementation
3. Wave 1 frontend domain cards
4. Wave 1 UAT

### Build second
5. Blood, iron & oxygen aggregation
6. Kidney domain enhancement
7. Thyroid scoring/domain implementation
8. 6-domain frontend extension
9. 6-domain UAT

### Reassess later
10. Silent inflammation feasibility
11. Hormone balance feasibility
...
Phase 2 Sprints 5–7 are largely independent and may be run in parallel if branch and contract coordination are managed carefully.
