...
**Important taxonomy clarification:**
This section must not assume that all surfaced entities are “phenotypes.” Based on current commercial and medical alignment, this layer should support multiple governed interpretation classes:
- phenotype (where scientifically valid)
- risk construct
- syndrome/state
- organ-pattern

Each card must still present a unified UX, but the underlying classification must remain accurate and governable.


**Purpose:**  
Show the structured interpretation layer that sits beneath the lead finding and above the biomarker evidence. This layer should not assume all entities are 'phenotypes'—it must support multiple scientifically valid classes (phenotype, risk construct, syndrome/state, organ-pattern).
...
- Determines whether a governed interpretation-display layer exists with the following fields: internal id, scientific classification (phenotype / risk / syndrome / organ-pattern), clinical display name, plain-English subtitle, and why-it-matters line
...
**Priority signals for extension (must prioritise highest commercial screening domains first):
- dysglycaemia / insulin resistance
- atherogenic cardiometabolic risk
- CKD / kidney risk
- MASLD / liver fibrosis risk
- integrated metabolic clustering
- iron deficiency (secondary high-volume lane)
