...
*Version 1.4 — Reviewed and amended against live codebase execution reality through KB-S48a, with intelligence-model governance alignment, upstream research-contract alignment, and intervention-effects registry foundation recorded as governing execution amendments*

> **Review note (v1.4):** This version preserves the strategic direction of v1.3 but records the foundational execution amendments that became necessary once live sprint execution exposed contract and governance gaps. Since v1.3, the codebase has introduced a locked intelligence-model schema, aligned the Knowledge Bus SOP and investigation-spec authoring contract to that schema, and established a governed intervention-effects registry foundation inside the Knowledge Bus. These amendments do not change the business strategy of breadth/depth braiding; they correct execution sequencing and make explicit the architectural foundations future sprints must now respect. All changes remain additive and are intended to make the roadmap more legible for future team members.

## Version Notes

- **v1.4:** records the post-v1.3 execution amendments required by live codebase maturity work. Specifically: (1) the intelligence-model schema was hardened and locked as a governed contract; (2) the Knowledge Bus SOP and upstream investigation-spec authoring contract were aligned to that new reality; (3) the intervention-effects registry foundation, alias-map skeleton, and standalone validator were introduced inside the Knowledge Bus; and (4) the original breadth/depth roadmap is explicitly reaffirmed as strategically correct, while execution sequencing is clarified to show that foundational maturity sprints were inserted before further scale-up. This version does not replace the strategic braid; it documents what the platform taught us on the way.
...
- KB-S45 delivery chain through remediation and regeneration against hardened contract
- KB-S47a / KB-S47b — target intelligence-model schema introduction and contract lock
- KB-S47c — Knowledge Bus SOP and upstream investigation-spec contract alignment
- KB-S48a — intervention-effects registry foundation, alias-map skeleton, and standalone validator
...
It also means the platform now has stronger architectural foundations than were assumed when v1.3 was authored:
- a locked intelligence-model schema
- an aligned upstream investigation-spec v3 authoring contract
- a KB SOP that reflects the new governed package and validation reality
- a governed intervention-effects registry foundation inside the Knowledge Bus
...
### 4.5 Execution amendments since v1.3

The strategic direction of the roadmap remains correct, but live execution exposed several foundational gaps that had to be corrected before breadth/depth expansion could safely continue.

Those gaps were not “nice to have” cleanups. They were structural issues in:
- schema shape
- package governance
- upstream research-authoring contract alignment
- validator authority
- medication / intervention modelling

As a result, a set of maturity sprints had to be inserted beneath the original Wave 1 logic. The most important of these were:

- intelligence-model schema hardening and lock
- Knowledge Bus SOP alignment to current package reality
- investigation-spec schema and research prompt-template alignment
- intervention-effects registry foundation and alias-map groundwork

These additions do **not** change the business strategy of alternating breadth and WHY depth.

They do change execution sequencing in one important way:

**Foundational maturity work now sits explicitly between early ingestion and later scale-up.**

The practical rule for future team members is:

- preserve the original strategic braid
- but do not resume breadth/depth expansion blindly if a foundational contract or governance gap is uncovered
- architecture hardening takes precedence when it protects future scale from hidden rework

This means the roadmap should now be read as:
- original strategic intent remains governing
- execution reality may insert bounded maturity sprints where the codebase proves they are necessary
- those maturity sprints are part of staying on plan, not drifting from it

### 4.6 Medication context and intervention architecture reality

v1.3 correctly established that Phase 1 medication handling must remain inside an explicitly bounded interpretation-caveat model unless a later sprint authorises deeper medication-aware reasoning.

That strategic boundary still stands.

What has now changed is that the platform has a clearer implementation architecture for that boundary:

- HealthIQ is **not** building a drug library
- canonical intervention knowledge is handled through a **class-level intervention-effects registry**
- that registry is governed **inside the Knowledge Bus**
- user intervention / exposure records remain **separate from canonical knowledge**
- later reasoning work will link the two rather than embedding medication data directly inside biomarker packages

This is an important execution amendment because future team members should not infer from the older roadmap wording that medication handling remains only a vague caveat concept. It now has a concrete governed architectural foundation.
...
- they should remain legible alongside the newer intervention-effects registry foundation, which governs canonical intervention knowledge rather than user-state capture
...
**Important execution note for future team members:** the wave sequence below remains strategically correct, but actual execution since v1.3 has already required bounded maturity/governance sprints to harden the platform foundation. Those sprints should be understood as enabling work that protects the roadmap, not as abandonment of the roadmap. Where the codebase reveals contract or governance instability, bounded hardening work may be inserted before breadth/depth expansion resumes.
...
- where medication/intervention knowledge is needed in canonical form, it should be supplied through the governed class-level intervention-effects registry rather than through package-local drug logic or an unbounded drug-library model
