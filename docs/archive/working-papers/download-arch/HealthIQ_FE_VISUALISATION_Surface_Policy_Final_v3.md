...
- a future optional symptom-relevance block should be reserved in the FE architecture, but not activated without separate governance
...
- one lead concern **when the evidence clearly supports a distinct lead**
- **or two or more co-primary / ranked-ambiguous concerns when the evidence picture does not justify forcing a single winner**
- a concise explanation of why the concern or concerns matter
- body-system context
- next-step logic
...
Where ambiguity exists, InsightPanel may still foreground one interpretation if it is the best-supported current lead, but it must make clear that:
...
Where two interpretations are materially tied, the standard surface must be allowed to show both and explain why the platform cannot honestly rank one above the other.
...
- explicit explanation of why one interpretation is ranked above another, or why two interpretations remain co-primary

### Governance decision
InsightPanel should be treated as the **primary product surface** in FE-VISUALISATION.

It must follow the same honesty rule across standard and clinician-facing outputs:
- where one interpretation is materially better supported, it may be surfaced as the lead concern
- where the evidence does not justify a single winner, the surface must allow co-primary or ranked-ambiguous output rather than forcing false certainty
...
## 5.5 Future optional symptom-relevance block

### Purpose
This is a **reserved future secondary interpretation slot**.

Its purpose would be to help connect a user’s interpreted body-system pattern to common symptom or lived-experience associations that might be relevant to why they sought testing in the first place.

This is not the same as the main interpretation surface.
It is not intended to replace InsightPanel.
It is not intended to make diagnostic claims.

### What it may eventually do
If separately governed and activated later, it may surface:

- possible symptom relevance associated with a body-system pattern
- examples of how a pattern can sometimes show up in lived experience
- simple explanation of why a blood pattern might be relevant to a symptom the user is thinking about

### What it must not do
It must not imply:

- that a symptom is definitely present because a blood pattern exists
- that the blood pattern proves the cause of a symptom
- that an associated ailment is diagnosed from bloods alone
- that a compelling symptom association equals personalised proof

### Governance decision
A **future optional widget/interface slot** for symptom relevance should be reserved in the FE surface architecture now so it can be added later without redesign.

But it should remain:

- secondary, not hero content
- association-based, not diagnostic
- subject to separate governance and activation before any live user-facing use

This means the FE should leave room for such a block beneath or adjacent to the main interpretation surface, but should not populate or expose it until a later governed decision authorises it.
...
5. Future optional symptom-relevance block
6. Internal/admin pipeline tooling later or internal-only
...
- reserved future optional symptom-relevance slot, not yet activated
...
- future symptom-relevance associations only if separately governed and clearly non-diagnostic
