...
*Version 1.3 — Reviewed and amended against live codebase audit 2026-03-21, with lifestyle/context-input roadmap clarification*

> **Review note (Claude Code, 2026-03-21):** Plan reviewed against live codebase audit (`docs/AUDIT_SPRINT_PLAN_2026-03-20.md`) and metabolic pathway coverage audit (`docs/METABOLIC_PATHWAY_COVERAGE_AUDIT_2026-03-20.md`). Four amendments applied: KB-S39 prerequisite added (§0); KB-S54 risk classification confirmed HIGH; BE-S1 risk classification corrected to STANDARD; KB-S58 scope-splitting note added. This version also adds an explicit governed lifestyle/questionnaire context workstream so high-impact non-biomarker inputs are treated as part of the platform architecture rather than background detail. No architectural conflicts with existing intelligence found. All sprints in this plan are additive. See individual amendment notes inline.

## Version Notes

- **v1.3:** adds the governed lifestyle / questionnaire / medication context workstream; inserts BE-S0 before narrative enablement; clarifies context-governance rules; carries forward the KB-S39 prerequisite, KB-S54 HIGH classification, BE-S1 classification correction, and KB-S58 scope-boundary rule.
...
## 0. Mandatory Prerequisite Before This Roadmap Begins

**KB-S39 must be completed, audited, and merged to main before KB-S45 is authored.**

The current working branch (`feature/kb-s39-edge-validator-hardfail-v1`) has uncommitted or unmerged modifications to several HIGH-risk files including `backend/core/analytics/ratio_registry.py`, `backend/core/contracts/insight_graph_v1.py`, `backend/scripts/run_work_package.py`, and multiple insight modules. These must not be in-flight when a new sprint begins.

Required sequence:
1. Complete KB-S39 audit and gate
2. Merge KB-S39 to main
3. Perform a clean branch checkout
4. Only then author KB-S45

This is non-negotiable per SOP §12 (Infrastructure/Control-Plane Deferral) and the standing rule against concurrent in-flight modifications to control-plane scripts.

In parallel with KB-S39 completion: **begin the renal interaction map research promotion process now.** Do not wait until KB-S56 is reached — by then it will be too late to avoid a blocked sprint.
...
Questionnaire / lifestyle / medication-context hardening work is governed as backend or mixed product-integration work, with risk level confirmed at prompt stage based on touched files.
...
## BE-S0 — Questionnaire / Lifestyle Context Hardening
**Type:** backend / MIXED / confirmed at hardening based on touched files  
**Purpose:** elevate high-impact non-biomarker inputs into an explicit governed platform input layer before narrative enablement.  
**Scope:**
- identify the minimum high-impact context set for Phase 1
- confirm those inputs are typed, governed, and intentionally consumed
- distinguish how numeric/objective inputs are governed versus how subjective/categorical inputs are governed
- verify alignment between questionnaire capture, mapper logic, overlay logic, and downstream analytical/narrative use
- harden the contract for the context inputs that materially influence interpretation

**Minimum high-impact context set for this sprint:**
...
**Why here:**
- these inputs materially affect deterministic metabolic interpretation
- the platform should not enable user-facing narrative before this context layer is explicitly governed
- this sprint prevents silent drift between collected questionnaire fields and actual engine/narrative consumption

**Rules:**
- no free-text lifestyle reasoning in the frontend
- no vague “questionnaire exists therefore context is handled” assumption
- context inputs must be intentionally mapped, governed, and reviewable
- medication handling must remain inside the Phase-1 caveat boundary above
- if touched-file scope crosses into analytical/runtime behavioural generation, escalate classification at hardening accordingly
