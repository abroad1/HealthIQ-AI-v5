 ...
 */
export function BalancedSystemsSummary({ balanced }: BalancedSystemsSummaryProps) {
  if (!balanced || !balanced.items?.length) return null;
    ...
    <Card className="border-emerald-100 bg-emerald-50/40 shadow-sm" data-testid="balanced-systems-summary">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg font-semibold text-emerald-950 flex items-center gap-2">
          ...
          Systems that look broadly stable on this panel
        </CardTitle>
      ...
      <CardContent className="space-y-4 text-sm text-emerald-950">
        <ul className="space-y-3">
          {balanced.items.map((it, idx) => (
            <li key={`${it.system_topic}-${idx}`} className="border-l-2 border-emerald-200 pl-3">
