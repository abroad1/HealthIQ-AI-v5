...
Each object should include at least one genuinely useful `differential` supporting marker wherever clinically feasible.

A `differential` marker must do real discriminative work:
- help distinguish the target condition frame from a nearby alternative
- narrow interpretation
- weaken or redirect competing explanations when absent or opposite

Do not use decorative or token differential markers just to satisfy the schema.
Do not mislabel a corroborator or mechanism marker as `differential` unless it truly functions that way.

If a frame cannot support a genuinely defensible differential marker, treat that as a weakness in the object and either:
- improve the frame
- refine the condition scope
- or exclude the frame if it cannot be made robust enough for the current product bar
...
1. Does each object conform to the schema structure?
2. Is every required section present?
3. Are claims calibrated to evidence strength?
4. Is the tone clinically neutral?
5. Are thresholds not invented or over-precise?
6. Are override conditions logically consistent?
7. Are supporting markers genuinely relevant?
8. Does every object include at least one genuinely justified `differential` supporting marker where clinically feasible?
...
DHEA (dhea) 
eGFR (egfr) 
Eosinophil Percentage (eosinophil_pct) 
Absolute Eosinophils (eosinophils_abs) 
Free Androgen Index (fai) 
Free T3 (free_t3) 
Free T4 (free_t4) 
Free Testosterone (free_testosterone) 
Free Testosterone Percentage (free_testosterone_pct)
