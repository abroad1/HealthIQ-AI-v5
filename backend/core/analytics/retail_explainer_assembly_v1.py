...
def attach_retail_explainers_v1(
    ...
    *,
    ...
    If the registry cannot be loaded or assembly fails, returns the original lists unchanged so
    analysis execution is not blocked (B1B pipeline safety).
    """
    try:
        ...
        logger.exception(
            "Retail explainer attachment skipped; continuing analysis without educational overlays.",
        )
        return list(biomarkers), list(clusters)
