...
class TestAnalysisService:
    ...
    async def test_start_analysis_success(self, service, sample_biomarkers, sample_user_data):
        ...
        with patch.object(service.orchestrator.normalizer, 'normalize_biomarkers') as mock_normalize, \
    ...
    async def test_validate_biomarker_panel_with_errors(self, service):
        ...
        with patch.object(service.orchestrator.normalizer, 'normalize_biomarkers') as mock_normalize, \
            ...
            # Mock validation to return high status for total_cholesterol to generate recommendations
            def mock_validate_side_effect(biomarker_name, value, unit):
                if biomarker_name == "total_cholesterol":
                    return {"status": "high"}
                else:
            ...
            # The current implementation converts non-numeric values to 0.0, so it won't fail validation
