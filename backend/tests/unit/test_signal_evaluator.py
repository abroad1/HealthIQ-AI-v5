...
def test_kb_s45a_lab_range_boundary_override_lower_triggers_against_lab_authority():
    """Canonical regression: lab_range_boundary + boundary lower uses lab_ranges[metric_id].min."""

    class _LabBoundaryRegistry:
        @staticmethod
        def get_all_signals():
            return [
                {
                    "signal_id": "signal_kb_s45a_lower",
                    "system": "hormonal",
                    "primary_metric": "tsh",
                    "supporting_metrics": [],
                    ...
                    "override_rules": [
                        {
                            "rule_id": "or_ft4_below_lab_min",
                            "description": "Escalate when Free T4 is below reference min.",
                            "conditions": [
                                {
                                    "metric_id": "free_t4",
    ...
    evaluator = SignalEvaluator(_LabBoundaryRegistry())
    out = evaluator.evaluate_all(
        signal_biomarkers={"tsh": 0.2, "free_t4": 8.0},
        signal_derived={},
        lab_ranges={
            "tsh": {"min": 0.4, "max": 4.5},
...
def test_kb_s45a_lab_range_boundary_override_upper_triggers_against_lab_authority():
    """Canonical regression: lab_range_boundary + boundary upper uses lab_ranges[metric_id].max."""
    ...
    class _LabBoundaryRegistry:
        ...
        def get_all_signals():
            return [
                {
                    "signal_id": "signal_kb_s45a_upper",
                    "system": "metabolic",
                    ...
                    "override_rules": [
                        {
                            "rule_id": "or_crp_above_lab_max",
                            "description": "Escalate when CRP is above lab max.",
                            "conditions": [
                                {
                                    "metric_id": "crp",
                                    "condition_type": "any_of",
    ...
    evaluator = SignalEvaluator(_LabBoundaryRegistry())
    out = evaluator.evaluate_all(
        signal_biomarkers={"glucose": 85.0, "crp": 5.0},
        signal_derived={},
        lab_ranges={"glucose": {"min": 70.0, "max": 99.0}, "crp": {"min": 0.0, "max": 3.0}},
    )
    assert len(out) == 1
    assert out[0].signal_state == "at_risk"


def test_kb_s45a_lab_range_boundary_override_below_min_alias_matches_lower():
    class _Reg:
        @staticmethod
        def get_all_signals():
            return [
                {
                    "signal_id": "signal_alias",
                    "system": "metabolic",
                    "primary_metric": "glucose",
                    "supporting_metrics": [],
                    "thresholds": [{"severity": "suboptimal", "operator": "range", "min_value": 70, "max_value": 99}],
    ...
    out = ev.evaluate_all(
        signal_biomarkers={"glucose": 80.0, "x": 1.0},
        signal_derived={},
        lab_ranges={"glucose": {"min": 70, "max": 99}, "x": {"min": 5.0, "max": 10.0}},
    )
    assert out[0].signal_state == "at_risk"


def test_kb_s45a_lab_range_boundary_out_of_range_either_side():
    class _Reg:
        @staticmethod
        def get_all_signals():
            return [
                {
                    "signal_id": "signal_oor",
                    "system": "metabolic",
                    "primary_metric": "glucose",
                    "supporting_metrics": [],
                    "thresholds": [{"severity": "suboptimal", "operator": "range", "min_value": 70, "max_value": 99}],
    ...
    ev = SignalEvaluator(_Reg())
    low = ev.evaluate_all(
        signal_biomarkers={"glucose": 80.0, "z": 1.0},
        signal_derived={},
        lab_ranges={"glucose": {"min": 70, "max": 99}, "z": {"min": 5.0, "max": 10.0}},
    )
    high = ev.evaluate_all(
        signal_biomarkers={"glucose": 80.0, "z": 12.0},
        signal_derived={},
        lab_ranges={"glucose": {"min": 70, "max": 99}, "z": {"min": 5.0, "max": 10.0}},
    )
    assert low[0].signal_state == "at_risk"
...
def test_kb_s45a_numeric_override_explicit_numeric_value_matches_legacy():
    ...
    ev = SignalEvaluator(_Reg())
    out = ev.evaluate_all(
        signal_biomarkers={"glucose": 85.0, "crp": 3.5},
        signal_derived={},
        lab_ranges={},
    )
    assert out[0].signal_state == "at_risk"
...
def test_kb_s45a_lab_boundary_override_evaluation_is_deterministic():
    class _Reg:
        @staticmethod
        def get_all_signals():
            return [
                {
                    "signal_id": "signal_det",
                    "system": "metabolic",
                    "primary_metric": "glucose",
                    "supporting_metrics": [],
                    "thresholds": [{"severity": "suboptimal", "operator": "range", "min_value": 70, "max_value": 99}],
    ...
    a = ev.evaluate_all(**kwargs)
    b = ev.evaluate_all(**kwargs)
...
_KB_S24_SIGNAL_CASES = {
    ...
    "signal_albumin_low": {
        ...
        "baseline_biomarkers": {"albumin": 30.0, "crp": 2.0, "alt": 25.0, "calcium": 2.3},
        "escalation_biomarkers": {"albumin": 30.0, "crp": 2.0, "alt": 70.0, "calcium": 2.3},
        "lab_ranges": {"albumin": {"min": 35.0, "max": 50.0}},
    },
    "signal_alp_high": {
        "no_trigger_biomarkers": {"alp": 100.0, "ggt": 30.0, "bilirubin": 10.0, "calcium": 2.3},
        "baseline_biomarkers": {"alp": 160.0, "ggt": 30.0, "bilirubin": 10.0, "calcium": 2.3},
        "escalation_biomarkers": {"alp": 160.0, "ggt": 70.0, "bilirubin": 30.0, "calcium": 2.3},
        "lab_ranges": {"alp": {"min": 30.0, "max": 120.0}},
    },
    "signal_calcium_high": {
        "no_trigger_biomarkers": {"calcium": 2.4, "albumin": 42.0, "vitamin_d": 90.0, "creatinine": 85.0},
        "baseline_biomarkers": {"calcium": 2.8, "albumin": 42.0, "vitamin_d": 90.0, "creatinine": 85.0},
        "escalation_biomarkers": {"calcium": 3.2, "albumin": 42.0, "vitamin_d": 90.0, "creatinine": 85.0},
        "lab_ranges": {"calcium": {"min": 2.2, "max": 2.6}},
    },
    ...
    "signal_folate_low": {
        "no_trigger_biomarkers": {"folate": 12.0, "vitamin_b12": 300.0, "mcv": 90.0, "homocysteine": 10.0},
        "baseline_biomarkers": {"folate": 5.0, "vitamin_b12": 300.0, "mcv": 90.0, "homocysteine": 10.0},
        "escalation_biomarkers": {"folate": 5.0, "vitamin_b12": 300.0, "mcv": 100.0, "homocysteine": 10.0},
        "lab_ranges": {"folate": {"min": 7.0, "max": 20.0}},
    },
    "signal_ggt_high": {
        "no_trigger_biomarkers": {"ggt": 40.0, "alt": 25.0, "alp": 100.0, "mcv": 90.0},
        "baseline_biomarkers": {"ggt": 80.0, "alt": 25.0, "alp": 100.0, "mcv": 90.0},
        "escalation_biomarkers": {"ggt": 80.0, "alt": 25.0, "alp": 150.0, "mcv": 90.0},
        "lab_ranges": {"ggt": {"min": 0.0, "max": 55.0}},
    },
    "signal_hba1c_high": {
    ...
    "signal_hdl_cholesterol_high": {
        "no_trigger_biomarkers": {"hdl_cholesterol": 1.8, "crp": 2.0},
        "baseline_biomarkers": {"hdl_cholesterol": 2.35, "crp": 2.0},
        "escalation_biomarkers": {"hdl_cholesterol": 2.6, "crp": 2.0},
        "lab_ranges": {"hdl_cholesterol": {"min": 1.0, "max": 2.3}},
    },
    "signal_hdl_cholesterol_low": {
        "no_trigger_biomarkers": {"hdl_cholesterol": 1.2, "triglycerides": 1.2, "hba1c": 35.0},
        "baseline_biomarkers": {"hdl_cholesterol": 0.8, "triglycerides": 1.2, "hba1c": 35.0},
        "escalation_biomarkers": {"hdl_cholesterol": 0.8, "triglycerides": 2.2, "hba1c": 35.0},
        "lab_ranges": {"hdl_cholesterol": {"min": 1.0, "max": 2.3}},
    },
    "signal_hemoglobin_low": {
        "no_trigger_biomarkers": {"hemoglobin": 135.0, "mcv": 90.0, "rdw_cv": 13.0},
        "baseline_biomarkers": {"hemoglobin": 100.0, "mcv": 90.0, "rdw_cv": 13.0},
    ...
    "signal_lymphocytes_high": {
        "no_trigger_biomarkers": {"lymphocytes": 3.0, "white_blood_cells": 7.0, "crp": 2.0},
        "baseline_biomarkers": {"lymphocytes": 4.5, "white_blood_cells": 7.0, "crp": 2.0},
